...
**Last updated:** 2026-08-07 (site-vitrine 3K-Négoce, gammes admin, ManoMano Toolbox, Accueil commandes filtrées — voir changelog)
...
6. **Changelog (obligatoire) :** à chaque **push** sur la branche **`staging`** et à chaque **push** sur **`main`** (ou merge équivalent vers production) pour **`fixa-pro-hub`**, **`admin-hub-central`** et/ou **`site-vitrine`**, ajouter **une ligne datée** dans le tableau **[Changelog](#changelog)** ci-dessous **et** une section datée dans **[`docs/CHANGELOG.md`](../admin-hub-central/docs/CHANGELOG.md)** (blocs `extract-start` / `extract-end` pour extraction par date). Résumé factuel : dépôt, branche, migrations Supabase, `db push`, Edge deploy, Railway rebuild, copie Storage, etc. Même principe après une opération infra manuelle significative si elle n’est pas déjà couverte par un commit le même jour. Format dates **AAAA-MM-JJ**, entrées **concises**.
...
| -------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `fixa-pro-hub/` (ce dépôt) | Storefront Vis-à-Bois (Vite/React) + in-app admin routes; Stripe checkout; many Supabase Edge Functions | `[https://github.com/3K-Negoce-Organisation/fixa-pro-hub.git](https://github.com/3K-Negoce-Organisation/fixa-pro-hub.git)`           |
| `admin-hub-central/` | Back-office (commandes, produits, catégories, fournisseur, utilisateurs, charte, marketplaces)             | `[https://github.com/3K-Negoce-Organisation/admin-hub-central.git](https://github.com/3K-Negoce-Organisation/admin-hub-central.git)` |
| `site-vitrine/` | Storefront marketplace **3K-Négoce** (même stack que fixa ; slug `3k-negoce`) | `[https://github.com/3K-Negoce-Organisation/site-vitrine.git](https://github.com/3K-Negoce-Organisation/site-vitrine.git)` |
...
| Date       | Change                                                                                                                                                                                                                                                                                       |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-07 | **site-vitrine :** client Supabase versionné (build Railway prod) ; merge staging→main. **admin :** catalogue `category_product` + exports ManoMano ; seed site `3k-negoce`. Voir [`admin-hub-central/docs/CHANGELOG.md`](../admin-hub-central/docs/CHANGELOG.md#2026-08-07). |
| 2026-08-06 | **admin :** Accueil filtre commandes par MP ; ManoMano Toolbox ; Produits = Catalogue. Voir CHANGELOG admin. |
| 2026-08-05 | **admin / n8n :** Accueil 50 commandes ; DPD PREDICT ; ManoMano eco/shippings. |
| 2026-08-04 | **admin :** ManoMano accept/refuse + tracking ; synchro listings publiés. |
| 2026-08-01 | **site-vitrine :** naissance storefront marketplace 3K-Négoce (gammes, layouts 1–17, header, Railway). **admin :** hub Sites / visuels, Railway dialog, sidebar 3 largeurs. |
| 2026-07-30 | **admin :** gammes catalogue + Edge `manage-gammes`. **site-vitrine :** pointage Supabase staging. |
| 2026-07-29 | **admin :** parser stock Alsafix nightly + deploy `import-alsafix-stock` prod. |
| 2026-07-28 | **admin :** feeds ManoMano Toolbox + branding Auth Hub Central. |
...
| 2026-06-03 | **SEO vis à bois (fixa) :** meta dynamiques, canonical `.com`, OG brandé, sitemap, JSON-LD, maillage interne ; **Git** PR **#4** **staging**→**main** `f336877`. **Edge prod** `sitemap`. **Railway** rebuild vitrine. Voir [`docs/CHANGELOG.md`](../docs/CHANGELOG.md#2026-06-03). |

...
| Date       | Change                                                                                                                                                                                                                                                                                       |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-23 | **fixa-pro-hub — prix monétaires :** `roundMoney` (front `src/lib/utils.ts`, Edge `_shared/money.ts`, `checkout-totals.ts`, `order-supplier-quantity.ts`) ; admin embarqué + panier + checkout ; migration `20260523150000_round_product_prices_clear_user_carts.sql` + workflow `run-sql-migration.yml` ; docs `DATA_CLEANUP_PRICES_CARTS.md`, `AGENT_SECRETS_FROM_IMAC.md`. **Git** staging → **main**. **SQL** staging + prod. **Edge** deploy checkout + webhooks. |

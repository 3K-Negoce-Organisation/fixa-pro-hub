-- KPMC — lecture public.orders / order_items pour le menu Factures clients
...
DROP POLICY IF EXISTS "KPMC factures_clients read orders" ON public.orders;
CREATE POLICY "KPMC factures_clients read orders"
  ON public.orders
  FOR SELECT
  TO authenticated
  USING (kpmc.user_can_access_page('factures_clients'));

DROP POLICY IF EXISTS "KPMC factures_clients read order_items" ON public.order_items;
CREATE POLICY "KPMC factures_clients read order_items"
